...
describe('Edge Cases and Complex Scenarios', () => {
  ...
  const edgeCasesSchemaPath = join(
    process.cwd(),
    'tests',
    'schemas',
    'edge-cases.prisma',
  ...
  it('should handle models with very long names', async () => {
    ...
    const longNameRouter =
      routers.modelRouters[
        'ModelWithVeryLongNameThatShouldTestNamingConventions'
      ];
    if (longNameRouter) {
      const structure =
        TrpcGeneratorTestUtils.validateRouterStructure(longNameRouter);
  ...
  it('should handle self-referencing models', async () => {
    ...
    if (selfRefRouter) {
      const structure =
        TrpcGeneratorTestUtils.validateRouterStructure(selfRefRouter);
      expect(structure.hasRouter).toBe(true);
  ...
  it('should handle circular relationships', async () => {
    ...
    if (circularARouter && circularBRouter) {
      const structureA =
        TrpcGeneratorTestUtils.validateRouterStructure(circularARouter);
      const structureB =
        TrpcGeneratorTestUtils.validateRouterStructure(circularBRouter);
  ...
  it('should handle all field types correctly', async () => {
    ...
    if (allTypesRouter) {
      const structure =
        TrpcGeneratorTestUtils.validateRouterStructure(allTypesRouter);
      expect(structure.hasRouter).toBe(true);
      expect(structure.procedures.length).toBeGreaterThan(0);
  ...
  it('should handle unique constraints', async () => {
    ...
    if (uniqueRouter) {
      const structure =
        TrpcGeneratorTestUtils.validateRouterStructure(uniqueRouter);
      expect(structure.hasRouter).toBe(true);
    }
  });
  ...
  it('should handle complex indexes', async () => {
    ...
    if (indexRouter) {
      const structure =
        TrpcGeneratorTestUtils.validateRouterStructure(indexRouter);
      expect(structure.hasRouter).toBe(true);
    }
  });
  ...
  it('should handle large enums', async () => {
    ...
    if (enumRouter) {
      const structure =
        TrpcGeneratorTestUtils.validateRouterStructure(enumRouter);
      expect(structure.hasRouter).toBe(true);
      expect(structure.procedures.length).toBeGreaterThan(0);
    }
  });
  ...
  it('should handle special characters in field names', async () => {
    ...
    const specialCharsRouter =
      routers.modelRouters['ModelWithSpecialCharacters'];
    if (specialCharsRouter) {
      const structure =
        TrpcGeneratorTestUtils.validateRouterStructure(specialCharsRouter);
      expect(structure.hasRouter).toBe(true);
    }
  });
  ...
  it('should handle many-to-many with explicit join table', async () => {
    ...
    if (leftRouter && rightRouter && connectionRouter) {
      const leftStructure =
        TrpcGeneratorTestUtils.validateRouterStructure(leftRouter);
      const rightStructure =
        TrpcGeneratorTestUtils.validateRouterStructure(rightRouter);
      const connectionStructure =
        TrpcGeneratorTestUtils.validateRouterStructure(connectionRouter);
  ...
  it('should handle cascade delete relationships', async () => {
    ...
    if (parentRouter && childRouter) {
      const parentStructure =
        TrpcGeneratorTestUtils.validateRouterStructure(parentRouter);
      const childStructure =
        TrpcGeneratorTestUtils.validateRouterStructure(childRouter);
  ...
  it('should respect limited generateModelActions', async () => {
    ...
    for (const [modelName, routerContent] of Object.entries(
      routers.modelRouters,
    )) {
