...
describe('Performance Tests', () => {
  const schemas = [
    {
      name: 'basic',
      path: join(process.cwd(), 'tests', 'schemas', 'basic.prisma'),
    },
    {
      name: 'comprehensive',
      path: join(process.cwd(), 'tests', 'schemas', 'comprehensive.prisma'),
    },
    {
      name: 'edge-cases',
      path: join(process.cwd(), 'tests', 'schemas', 'edge-cases.prisma'),
    },
  ];

  it.each(schemas)(
    'should generate $name schema within performance limits',
    async ({ path }) => {
      const performance =
        await TrpcGeneratorTestUtils.testGenerationPerformance(path);
  ...
  it('should handle large schemas efficiently', async () => {
    ...
    const performance = await TrpcGeneratorTestUtils.testGenerationPerformance(
      comprehensiveSchemaPath,
    );
    ...
    console.log(
      `Comprehensive schema generation took ${performance.duration.toFixed(2)}ms`,
    );
  });

  it('should generate files of reasonable size', async () => {
    const comprehensiveOutputDir = join(
      process.cwd(),
      'tests',
      'generated',
      'comprehensive',
    );
    const routers = TrpcGeneratorTestUtils.readGeneratedRouters(
      comprehensiveOutputDir,
    );
    ...
    if (routers.appRouter) {
      const appRouterSizeKB =
        Buffer.byteLength(routers.appRouter, 'utf8') / 1024;
  ...
  it('should scale linearly with model count', async () => {
    ...
    const comprehensivePerf =
      await TrpcGeneratorTestUtils.testGenerationPerformance(
    ...
    console.log(
      `Performance ratio (comprehensive/basic): ${ratio.toFixed(2)}x`,
    );
  });
  ...
  it('should optimize repeated generations', async () => {
    ...
    const firstRun =
      await TrpcGeneratorTestUtils.testGenerationPerformance(schemaPath);
  ...
  it('should handle edge cases without performance degradation', async () => {
    ...
    const performance =
      await TrpcGeneratorTestUtils.testGenerationPerformance(edgeCasesPath);

    // Edge cases should not cause exponential slowdown
    expect(performance.duration).toBeLessThan(45000); // 45 seconds max

    console.log(
      `Edge cases generation took ${performance.duration.toFixed(2)}ms`,
    );
  });
  ...
  it('should measure router complexity impact', async () => {
    ...
    const basicRouters =
      TrpcGeneratorTestUtils.readGeneratedRouters(basicOutputDir);
    const comprehensiveRouters = TrpcGeneratorTestUtils.readGeneratedRouters(
      comprehensiveOutputDir,
    );
    ...
    for (const routerContent of Object.values(
      comprehensiveRouters.modelRouters,
    )) {
      const structure =
        TrpcGeneratorTestUtils.validateRouterStructure(routerContent);
